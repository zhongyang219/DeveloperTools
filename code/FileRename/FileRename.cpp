...
void FileRename::OnCommand(const char* strCmd, bool checked)
{
    ...
    else if (cmd == CMD_FileRenameAddFolder)
    {
		...
		if (!strDir.isEmpty())
		{
			QFileInfoList fileInfoList = CCommonTools::FindFile(strDir);
			Q_FOREACH(const QFileInfo & fileInfo, fileInfoList)
			{
				m_mainWidget.AddFile(fileInfo.filePath());
			}
		}
    }
    else if (cmd == CMD_FileRenameDelete)
    {
		auto selections = m_mainWidget.GetTableView()->selectionModel()->selectedIndexes();
